...
export default function GroggyLogCard({
  platform,
  matchId,
  playerName,
  mapName,
  ...
  const { data, isLoading, error } = useTelemetryGroggy(
    platform,
    matchId,
  ...
  const renderContent = (logs: GroggyLogData) => {
    if (logs.length === 0) {
      return (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          기절(DBNO) 로그 데이터가 없습니다.
        </div>
      );
    }

    const playerKnocks = logs.filter((l) => l.attacker.name === playerName);
    const playerDowned = logs.filter((l) => l.victim.name === playerName);

    const knockStats = logs.reduce(
      (acc, l) => {
        const name = l.attacker.name;
        acc[name] = (acc[name] || 0) + 1;
        return acc;
      },
      {} as Record<string, number>
    );
    const topKnocker = Object.entries(knockStats).sort((a, b) => b[1] - a[1])[0];
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-3 text-center border border-yellow-200 dark:border-yellow-800">
            <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{logs.length}</div>
            <div className="text-sm text-yellow-700 dark:text-yellow-300">총 기절</div>
          </div>
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center border border-blue-200 dark:border-blue-800">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{playerKnocks.length}</div>
            <div className="text-sm text-blue-700 dark:text-blue-300">내가 기절시킴</div>
          </div>
          <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 text-center border border-red-200 dark:border-red-800">
            <div className="text-2xl font-bold text-red-600 dark:text-red-400">{playerDowned.length}</div>
            <div className="text-sm text-red-700 dark:text-red-300">내가 기절됨</div>
          </div>
        </div>
        ...
        {topKnocker && viewMode === 'list' && (
          <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-3 border border-yellow-200 dark:border-yellow-800 flex items-center gap-3">
            <span className="text-2xl">👊</span>
            <div>
              <div className="text-sm text-yellow-700 dark:text-yellow-300">최다 기절</div>
              <div className="font-semibold text-gray-800 dark:text-gray-200">
                {topKnocker[0]}
                <span className="ml-2 text-yellow-600 dark:text-yellow-400">({topKnocker[1]}회)</span>
              </div>
            </div>
          </div>
        )}
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-gray-100 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="text-left p-2 text-gray-700 dark:text-gray-300">공격자</th>
                  ...
                  <th className="text-left p-2 text-gray-700 dark:text-gray-300">부위</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((entry, i) => {
                  const isMeAttacker = entry.attacker.name === playerName;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2 font-medium text-gray-800 dark:text-gray-200">
                        {isMeAttacker ? (
                          <span className="text-blue-600 dark:text-blue-400">★ {entry.attacker.name}</span>
                        ) : (
                          entry.attacker.name
                        )}
                      </td>
                      ...
                      <td className="p-2 text-gray-600 dark:text-gray-400 max-w-[120px] truncate">
                        {entry.weapon}
                      </td>
